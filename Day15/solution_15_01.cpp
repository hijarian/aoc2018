...
struct Map
{
  ...
  int count_total_hp();
  void damage(unsigned row, unsigned col);
};

void Map::damage(unsigned row, unsigned col)
{
  for (Unit &unit : this->units)
  {
    if (unit.row == row && unit.col == col)
    {
      unit.hp -= unit.representation == 'G' ? 14 : 3;
      return;
    }
  }
}
...
int main(int argc, char** argv)
{
  ...
  Map map = read_map(inputfilename);
  prepare_screen();
  ...
  while(!is_victory)
  {
    ...
    for (Unit &current_unit: map.units)
    {
      ...
      if (!enemies_in_range.empty())
      {
        ...
        map.damage(min_enemy_hp_row, min_enemy_hp_col);
